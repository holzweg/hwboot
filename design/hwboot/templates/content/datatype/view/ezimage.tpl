...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        {/if}
        {if $href}<a href={$href}{if and( is_set( $link_class ), $link_class )} class="{$link_class}"{/if}{if and( is_set( $popover ), $popover )} rel="popover" data-content='{$popover_content|wash()}' data-placement="below" {/if}{if and( is_set( $link_id ), $link_id )} id="{$link_id}"{/if}{if $target} target="{$target}"{/if}{if and( is_set( $link_title ), $link_title )} title="{$link_title|wash}"{/if}>{/if}
