...
{if ne( $pagedata.class_identifier, 'documentation_page' )}
    ...
    {if $left_menu_items_count}
        ...
        {foreach $left_menu_items as $key => $item}
            {* {set $a_class = cond($current_node_in_path_2|eq($item.node_id), array("active"), array()) } *}
                 {set $li_class = cond( $key|eq(0), array("firstli"), array() )}
				 {* @todo make icon classes configurable, this is just an example! *}
                 {set $icon_class = cond( $key|eq(0), "icon-home", "icon-list-alt" )}
            ...
            {if eq( $item.class_identifier, 'link')}
                <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $item.node_id)|ezurl}{else}href={$item.data_map.location.content|ezurl}{if and( is_set( $item.data_map.open_in_new_window ), $item.data_map.open_in_new_window.data_int )} target="_blank"{/if}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if} title="{$item.data_map.location.data_text|wash}" class="menu-item-link" rel={$item.url_alias|ezurl}><i class="{$icon_class}"></i>{if $item.data_map.location.data_text}{$item.data_map.location.data_text|wash()}{else}{$item.name|wash()}{/if}</a>
            {else}
                <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $item.node_id)|ezurl}{else}{$item.url_alias|ezurl}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if}><i class="{$icon_class}"></i>{$item.name|wash()}</a>
            {/if}
                ...
                <ul class="submenu-list">
                    {foreach $sub_menu_items as $subkey => $subitem}
                       {* {set $a_class = cond($current_node_in_path_3|eq($subitem.node_id), array("active"), array()) } *}
                        {set $li_class = cond( $subkey|eq(0), array("firstli"), array() )}
                    ...
                    {if eq( $subitem.class_identifier, 'link')}
                        <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $subitem.node_id)|ezurl}{else}href={$subitem.data_map.location.content|ezurl}{if and( is_set( $subitem.data_map.open_in_new_window ), $subitem.data_map.open_in_new_window.data_int )} target="_blank"{/if}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if} title="{$subitem.data_map.location.data_text|wash}" class="menu-item-link" rel={$subitem.url_alias|ezurl}><i class="{$icon_class}"></i>{if $subitem.data_map.location.data_text}{$subitem.data_map.location.data_text|wash()}{else}{$subitem.name|wash()}{/if}</a></li>
                    {else}
                        <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $subitem.node_id)|ezurl}{else}{$subitem.url_alias|ezurl}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if}><i class="{$icon_class}"></i>{$subitem.name|wash()}</a></li>
                    {/if}
                    {/foreach}
                </ul>
                {/if}
