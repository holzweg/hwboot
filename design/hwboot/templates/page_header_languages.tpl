...
{foreach $avail_translation as $siteaccess => $lang}
    <li{if $siteaccess|eq($access_type.name)} class="current_siteaccess"{/if}><a class="btn btn-small" href={$lang.url|ezurl}><i class="icon-chevron-right"></i>{$lang.text|wash}</a>{delimiter}{/delimiter}</li>
{/foreach}
